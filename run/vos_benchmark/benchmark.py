...
def validate_folders(
    ...
    gt_videos = list(
        filter(lambda x: path.isdir(path.join(gt_root, x)), gt_videos)
    )
    mask_videos = list(
    ...
    if not strict:
        videos = natsorted(list(set(gt_videos) & set(mask_videos)))
    else:
